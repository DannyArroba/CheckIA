...
function mapHackiaClaimForTable(claim) {
  return {
    ...
    policy_id: claim.id_poliza,
    insured_id: claim.id_asegurado,
    ...
    claim_date: claim.fecha_siniestro || '-',
    report_date: claim.fecha_reporte || '-',
    days_report: claim.dias_ocurrencia_reporte ?? '-',
    city: claim.ciudad || claim.sucursal || 'Sin ciudad',
    branch: claim.sucursal || '-',
    provider_id: claim.id_proveedor || '-',
    ...
    estimated_amount: Number(claim.monto_estimado || 0),
    paid_amount: Number(claim.monto_pagado || 0),
    status: claim.estado || '-',
    docs_complete: claim.docs_completos ? 'Si' : 'No',
    provider_restricted: claim.proveedor_lista_restrictiva ? 'Si' : 'No',
    days_from_start: claim.dias_desde_inicio_poliza ?? '-',
    days_to_end: claim.dias_hasta_fin_poliza ?? '-',
    previous_claims: claim.reclamos_previos ?? '-',
    insured_sum: Number(claim.suma_asegurada || 0),
    narrative_similarity: claim.similitud_narrativa_max ?? '-',
    police_report_number: claim.numero_parte_policial || '-',
    risk_score: Number(claim.puntaje_riesgo || 0),
    risk_level: claim.nivel_riesgo || 'Bajo',
    event_description: claim.descripcion_evento || '-',
