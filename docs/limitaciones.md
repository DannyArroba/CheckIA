# Limitaciones

- Los datos son completamente sintéticos.
- El modelo no está validado con datos reales.
- Las métricas son simuladas para fines de demostración.
- El score no es evidencia de fraude.
- El sistema no debe rechazar siniestros automáticamente.
- Requiere revisión humana, auditoría, gobierno de datos y validación legal antes de cualquier uso productivo.
- Los sesgos en datos reales podrían afectar resultados si no se controlan.
- La lista restrictiva de proveedores es simulada y no representa entidades reales.

Mensaje ético central:

> Esta alerta no constituye una acusación de fraude. El caso requiere revisión humana antes de cualquier decisión.
